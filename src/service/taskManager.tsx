...
class TaskManager {
    ...
    loadTasks(){
        ...
        return TasksArrayJson.map((task : Task)=> new Task(
            task.title,
            task.id,
            task.status,
            task.priority,
    ...
    addTasks (title : string, status : Status,priority: Priority,startDateString : string,dueDateString : string){
        const newTaskId = crypto.randomUUID()
        const startDate = new Date(startDateString)
        const dueDate = new Date(dueDateString)
        
        const newTask = new Task(title,newTaskId,status,priority,startDate,dueDate)
